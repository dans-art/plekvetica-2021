...
if ($type === 'edit' and PlekUserHandler::user_can_edit_organizer($organizer_id) !== true) {
    echo __('You are not allowed to edit this organizer!', 'pleklang');
    return;
}
?>
...
<?php if (!$only_input_fields) : ?>
    <div id='organizer-<?php echo $type; ?>-<?php echo $organizer_id ?: 'new'; ?>' class='organizer-<?php echo $type; ?> organizer-<?php echo $type; ?>-container'>
        <h1><?php echo ($type === 'add') ? __('Add Organizer', 'pleklang') : __('Edit Organizer', 'pleklang'); ?></h1>
        ...
        <?php if (!$only_input_fields) : ?>
            <div class="buttons">
                <button id="organizer-form-cancel" class="plek-button plek-button-cancel" type="button"><?php echo __('Cancel', 'pleklang'); ?></button>
                <button id="organizer-form-submit" class="plek-button" type="submit"><?php echo __('Save', 'pleklang'); ?></button>
            </div>
        </form>
    </div>
<?php endif; ?>