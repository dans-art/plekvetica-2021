...
<div>
    ...
    if ($plek_event->show_publish_button()) : ?>
        <a id="plekSetEventStatus" name="setEventStauts" class="plek-button full-width blue" data-eventid="<?php echo $event_id; ?>" data-type="publish_event"><?php echo __('Publish Event', 'pleklang'); ?></a>
    <?php endif; ?>
    <?php
    //Write Review Button    
    ...
    if (PlekUserHandler::current_user_can_edit($plek_event) and !$plek_event->is_review() and $plek_event->has_revisions()) : ?>
        ...
        ?>
            <a name="revisionEvent" class="plek-button full-width <?php echo ''; ?> plekRevision" data-eventid="<?php echo $event_id . "," . $rev_id; ?>"><?php echo __("Show revision", "pleklang"); ?><div class='smallText'><?php echo $plek_event->get_revision_modified_date($rev_id); ?></div></a>
        <?php endforeach; ?>
    <?php endif; ?>
    ...
    if (PlekUserHandler::user_is_in_team() and !empty($akk_status) and !empty($akk_crew)) : ?>
        <dl class='event-akkredi-container'>
            <dt><?php echo __('Accreditation status', 'pleklang'); ?></dt>
            <dd class="event-akkredi-status">
                <span class="<?php echo $akk_status; ?>">
                    <?php if (!empty($akk_status)) {
                        ...
                        echo __('Event has not yet been accredited by any team member.', 'pleklang');;
                    } ?>
                </span>
            </dd>
            <dt><?php echo __('Accredited members', 'pleklang'); ?></dt>
    ...
    <?php
                    ...
                    echo ($plek_event->is_review()) ? __('This event did not have any interviews.', 'pleklang') : __('No interviews have yet been registered for this event.', 'pleklang');
                } ?>
    </dd>
        </dl>
