					...
					<div class="generate_disable_elements">
						...
						<?php if (!defined('GP_PREMIUM_VERSION')) : ?>
							<span style="display:block;padding-top:1em;border-top:1px solid #EFEFEF;">
								<a href="<?php echo generate_get_premium_url('https://generatepress.com/downloads/generate-disable-elements'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function. 
											?>" target="_blank"><?php esc_html_e('Premium module available', 'generatepress'); ?></a>
							</span>
						<?php endif; ?>
					</div>
				<?php endif; ?>
...
function generate_save_layout_meta_data($post_id)
{
	...
	$sidebar_layout_value = filter_input(INPUT_POST, htmlspecialchars($sidebar_layout_key));
	...
	$footer_widget_value = filter_input(INPUT_POST, htmlspecialchars($footer_widget_key));
	...
	$page_builder_container_value = filter_input(INPUT_POST, htmlspecialchars($page_builder_container_key));
	...
	if (!defined('GENERATE_DE_VERSION')) {
		...
		$disable_content_title_value = filter_input(INPUT_POST, htmlspecialchars($disable_content_title_key));
